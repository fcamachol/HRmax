...
**Authentication & Authorization**: Features a multi-tenant permission system with hierarchical scope resolution (cliente → empresa → centro_trabajo → módulo), `requirePermission` middleware, and session-based authentication with HttpOnly cookies. It supports internal (MaxTalent) and client user types with granular access control.
**Client Authentication System**: Session-based authentication using `express-session` with MemoryStore (development) and HttpOnly secure cookies. Endpoints: POST `/api/auth/login` (validates bcrypt password, creates session), GET `/api/auth/me` (returns current user from session), POST `/api/auth/logout` (destroys session). Frontend uses `AuthContext` for user state management and `RequireAuth` component for route protection. Login page at `/login` with automatic redirect for unauthenticated users.
...
**Basis Points Helper Library**: A shared library (`shared/basisPoints.ts`) provides functions for converting pesos to basis points and performing safe arithmetic with 4-decimal precision using `Math.trunc()` to prevent rounding errors. Includes `dividirBpRedondeado()` for precision-preserving division when converting monthly → daily amounts.
...
**Dual Compensation System (BRUTO/NETO)**: Complete compensation architecture supporting both gross and net salary schemes:
  - `compensacion_trabajador`: Anchor table with vigencia dates. Stores `esquema_tipo` (BRUTO/NETO) column as canonical source, `neto_deseado_bp`/`bruto_bp` as anchor, and package distribution percentages (% salario nominal, % exento, % bonos).
  ...
  - **Compensation Service** (`server/services/compensationService.ts`): Compatibility layer with `getSalarioParaNomina()` that reads from new compensation tables with automatic fallback to legacy employee records. Uses rounding division for daily salary derivation.
  - **Inverse Payroll Calculator** (`server/services/inversePayrollCalculator.ts`): Newton-Raphson style iteration to calculate BRUTO from NETO target. Uses actual marginal ISR bracket rates + IMSS rates for derivative estimation. Conditional damping only when derivative outside valid range [0,1]. Includes oscillation detection and safety bounds.
